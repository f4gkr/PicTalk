...
{
    close();
    if( HTTP_DEBUG ) qDebug("HttpListener: destroyed");
}
...
void HttpListener::listen()
{
    ...
    int port=settings->value("port","8001").toInt();
    if( port <= 0 )
        return ;
    ...
    if (!isListening())
    {
        qCritical("Web server cannot bind on port %i: %s",port,qPrintable(errorString()));
    }
    else {
        qDebug("Web server listening on port %i",port);
    }
}
...
void HttpListener::close() {
    QTcpServer::close();
    if( HTTP_DEBUG ) qDebug("HttpListener: closed");
    if (pool) {
        delete pool;
        pool=NULL;
    }
...
void HttpListener::incomingConnection(tSocketDescriptor socketDescriptor) {
    ...
    else
    {
        ...
        if( HTTP_DEBUG ) qDebug("HttpListener: Too many incoming connections");
