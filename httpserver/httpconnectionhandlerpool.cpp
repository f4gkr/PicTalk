...
HttpConnectionHandlerPool::~HttpConnectionHandlerPool()
{
    ...
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandlerPool (%p): destroyed", this);
}
...
{
    int maxIdleHandlers=settings->value("minThreads",20).toInt();
    int idleCounter=0;
    mutex.lock();
    foreach(HttpConnectionHandler* handler, pool)
    {
        if (!handler->isBusy())
        {
            if (++idleCounter > maxIdleHandlers)
            {
                ...
                if( HTTP_DEBUG ) qDebug("HttpConnectionHandlerPool: Removed connection handler (%p), pool size is now %i",handler,pool.size());
...
void HttpConnectionHandlerPool::loadSslConfig()
{
    ...
    if (!sslKeyFileName.isEmpty() && !sslCertFileName.isEmpty())
    {
            ...
            if( HTTP_DEBUG ) qDebug("HttpConnectionHandlerPool: SSL settings loaded");
         #endif
    }
}
