...
HttpRequest::HttpRequest(QSettings* settings)
{
    ...
    currentSize=0;
    expectedBodySize=0;
    maxSize=settings->value("maxRequestSize","128000").toInt();
    maxMultiPartSize=settings->value("maxMultiPartSize","128000").toInt();
}
...
void HttpRequest::readRequest(QTcpSocket* socket)
{
    ...
    if (!newData.isEmpty())
    {
        ...
        {
            if( HTTP_DEBUG ) qDebug() << newData ;
            if( HTTP_DEBUG ) qWarning("HttpRequest: received broken HTTP request, invalid first line");
            status=abort;
        }
        else {
...
void HttpRequest::readHeader(QTcpSocket* socket)
{
    ...
    else
    {
        ...
        else if (boundary.isEmpty() && Wait4Boundary && expectedBodySize+currentSize>maxSize)
        {
            if( HTTP_DEBUG ) qWarning("HttpRequest: expected body is too large");
            status=abort;
        }
        else if (!boundary.isEmpty() && expectedBodySize>maxMultiPartSize)
        {
            if( HTTP_DEBUG ) qWarning("HttpRequest: expected multipart body is too large");
            status=abort;
        }
        else {
            #ifdef SUPERVERBOSE
...
void HttpRequest::readBody(QTcpSocket* socket)
{
    ...
    else
    {
        ...
        {
            if( HTTP_DEBUG ) qWarning("HttpRequest: received too many multipart bytes");
            status=abort;
        }
...
void HttpRequest::replaceParameter( const QByteArray& name, QByteArray newValue ) {
     parameters.insert( name, newValue) ;
}
...
void HttpRequest::readFromSocket(QTcpSocket* socket)
{
    ...
    {
        if( HTTP_DEBUG ) qWarning("HttpRequest: received too many bytes");
        qDebug() << "boundary.isEmpty()=" << boundary.isEmpty() ;
        qDebug() << "currentSize=" << currentSize ;
        qDebug() << "maxSize=" << maxSize ;
        qDebug() << "maxMultiPartSize=" << maxMultiPartSize ;
        status=abort;
    }
    if (status==complete)
    {
...
void HttpRequest::replacePath( QByteArray newVal ) {
    path = newVal ;
    decodeRequestParams();
}
...
{
    if( HTTP_DEBUG ) qDebug("HttpRequest: parsing multipart temp file");
    tempFile.seek(0);
    bool finished=false;
    while (!tempFile.atEnd() && !finished && !tempFile.error())
    {
        ...
        while (!tempFile.atEnd() && !finished && !tempFile.error())
        {
            ...
            if (line.startsWith("Content-Disposition:"))
            {
                ...
                {
                    if( HTTP_DEBUG ) qDebug("HttpRequest: ignoring unsupported content part %s",line.data());
                }
            }
        ...
        while (!tempFile.atEnd() && !finished && !tempFile.error())
        {
            ...
            if (line.startsWith("--"+boundary))
            {
                ...
                if (fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    if( HTTP_DEBUG ) qDebug("HttpRequest: set parameter %s=%s",fieldName.data(),fieldValue.data());
                }
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    if( HTTP_DEBUG ) qDebug("HttpRequest: set parameter %s=%s",fieldName.data(),fileName.data());
                    uploadedFiles.insert(fieldName,uploadedFile);
                   if( HTTP_DEBUG )  qDebug("HttpRequest: uploaded file size is %i",(int) uploadedFile->size());
                }
