    ...
    :HttpRequestHandler(parent)
{
    ...
    if( DEBUG_CACHE ) qDebug("StaticFileController: docroot=%s, encoding=%s, maxAge=%i",qPrintable(docroot),qPrintable(encoding),maxAge);
    maxCachedFileSize=settings->value("maxCachedFileSize","65536").toInt();
    cache.setMaxCost(settings->value("cacheSize","1000000").toInt());
    cacheTimeout=settings->value("cacheTime","60000").toInt();
    if( DEBUG_CACHE ) qDebug("StaticFileController: cache timeout=%i, size=%i",cacheTimeout,cache.maxCost());
}
...
void StaticFileController::service(HttpRequest& request, HttpResponse& response)
{
    ...
    if (entry && (cacheTimeout==0 || entry->created>now-cacheTimeout))
    {
        ...
        mutex.unlock();
        if( DEBUG_CACHE ) qDebug("StaticFileController: Cache hit for %s",path.data());
    ...
    else
    {
        ...
        if( DEBUG_CACHE ) qDebug("StaticFileController: Cache miss for %s",path.data());
        ...
        if( DEBUG_CACHE ) qDebug("StaticFileController: Open file %s",qPrintable(file.fileName()));
