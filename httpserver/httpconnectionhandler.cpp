    ...
    : QThread()
{
    ...
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): constructed", this);
    this->start();
}
...
HttpConnectionHandler::~HttpConnectionHandler()
{
    quit();
    wait();
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): destroyed", this);
}
...
void HttpConnectionHandler::createSocket()
{
    // If SSL is supported and configured, then create an instance of QSslSocket
        ...
        if (sslConfiguration)
        {
            ...
            socket=sslSocket;
            if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): SSL is enabled", this);
            return;
        }
    #endif
...
{
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): thread started", this);
    try
    {
        exec();
    }
    ...
    delete socket;
    readTimer.stop();
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): thread stopped", this);
}


void HttpConnectionHandler::handleConnection(tSocketDescriptor socketDescriptor)
{
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): handle new connection", this);
    busy = true;
    ...
    if (!socket->setSocketDescriptor(socketDescriptor))
    {
        ...
        if (sslConfiguration)
        {
            if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): Starting encryption", this);
...
{
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): read timeout occured",this);
...
{
    if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): disconnected", this);
    socket->close();
    readTimer.stop();
    busy = false;
}
...
void HttpConnectionHandler::read()
{
    ...
    while (socket->bytesAvailable())
    {
        ...
        {
            readTimer.stop();
            if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): received request",this);
            ...
            if( HTTP_DEBUG ) qDebug("HttpConnectionHandler (%p): finished request",this);
