...
int mirisdr_open (mirisdr_dev_t **p, uint32_t index) {
    ...
    *p = NULL;
    ...
    libusb_init(&dev->ctx);
    libusb_set_debug(dev->ctx, LIBUSB_LOG_LEVEL_ERROR) ;
    i_max = libusb_get_device_list(dev->ctx, &list);

    for (i = 0; i < i_max; i++) {
        libusb_get_device_descriptor(list[i], &dd);
        mirisdr_device_t *devtype = mirisdr_device_get(dd.idVendor, dd.idProduct) ;
        if ((devtype != NULL) && (count++ == index)) {
            device = list[i];
            break;
        }
    }
    ...
    /* stop streaming in case it is running */
    ...
    dev->hw_flavour = MIRISDR_HW_SDRPLAY_RSP1;
